...
public class InvoiceServiceClient : IInvoiceServiceClient
{
    ...
    public async Task<(bool HasReferences, bool ServiceUnavailable)> CheckReferencesAsync(Guid supplierId, CancellationToken cancellationToken)
    {
        try
        {
            var response = await _httpClient.GetAsync($"/v1/invoices/references/{supplierId}", cancellationToken);
        ...
        {
            _logger.LogError(ex, "Failed to check Invoice references for supplier {SupplierId}", supplierId);
            return (false, true);
        }
    }
}
